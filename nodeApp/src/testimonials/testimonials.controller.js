...
function getAll(req, res, next) {
    testimonialService.getAll(req.body)
...
function getWhere(req, res, next) {
    if(req.params.where === 'where'){
        testimonialService.getWhere(req.params.whereClause)
        .then(testimonials => res.json(testimonials))
        .catch(next);
    }
    else {
...
function getById(req, res, next) {
    // users can get their own testimonial and admins can get any testimonial
...
function update(req, res, next) {
    // users can update their own testimonial and admins can update any testimonial
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }
...
function _delete(req, res, next) {
    // users can delete their own testimonial and admins can delete any testimonial
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    testimonialService.delete(req.params.id)
        .then(() => res.json({ message: 'testimonial deleted successfully' }))
        .catch(next);
}

function bulkCreate(req, res, next) {
    testimonialService.bulkCreate(req.body)
        .then(testimonial => res.json(testimonial))
        .catch(next);
}

function bulkDelete(req, res, next) {
    testimonialService.bulkDelete(req.body)
        .then(testimonial => res.json({message:"testimonial deleted successfully"}))
        .catch(next);
}
