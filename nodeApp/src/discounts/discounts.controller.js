...
function getAll(req, res, next) {
    discountService.getAll(req.body)
...
function getWhere(req, res, next) {
    if(req.params.where === 'where'){
        discountService.getWhere(req.params.whereClause)
        .then(discounts => res.json(discounts))
        .catch(next);
    }
    else {
...
function getById(req, res, next) {
    // users can get their own discount and admins can get any discount
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    discountService.getById(req.params.id)
...
function create(req, res, next) {
    discountService.create(req.body)
...
function update(req, res, next) {
    ...
    discountService.update(req.params.id, req.body)
        .then(discount => res.json(discount))
        .catch(next);
}

function _delete(req, res, next) {
    // users can delete their own discount and admins can delete any discount
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    discountService.delete(req.params.id)
        .then(() => res.json({ message: 'discount deleted successfully' }))
        .catch(next);
}

function bulkCreate(req, res, next) {
    discountService.bulkCreate(req.body)
        .then(discount => res.json(discount))
        .catch(next);
}

function bulkDelete(req, res, next) {
    discountService.bulkDelete(req.body)
        .then(discount => res.json({message:"discount deleted successfully"}))
        .catch(next);
}
