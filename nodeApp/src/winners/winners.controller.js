...
function getWhere(req, res, next) {
    if(req.params.where === 'where'){
        winnerService.getWhere(req.params.whereClause)
        .then(winners => res.json(winners))
        .catch(next);
    }
    else {
...
function getById(req, res, next) {
    // users can get their own winner and admins can get any winner
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    winnerService.getById(req.params.id)
        .then(winner => winner ? res.json(winner) : res.sendStatus(404))
        .catch(next);
}
...
function update(req, res, next) {
    // users can update their own winner and admins can update any winner
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    winnerService.update(req.params.id, req.body)
        .then(winner => res.json(winner))
        .catch(next);
}

function _delete(req, res, next) {
    // users can delete their own winner and admins can delete any winner
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    winnerService.delete(req.params.id)
        .then(() => res.json({ message: 'winner deleted successfully' }))
        .catch(next);
}

function bulkCreate(req, res, next) {
    winnerService.bulkCreate(req.body)
        .then(winner => res.json(winner))
        .catch(next);
}
