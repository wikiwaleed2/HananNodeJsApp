...
const cashAlternativeService = require('./cash-alternative.service');
...
function getAll(req, res, next) {
    cashAlternativeService.getAll(req.body)
        .then(cashAlternatives => res.json(cashAlternatives))
        .catch(next);
}
...
function getWhere(req, res, next) {
    if(req.params.where === 'where'){
        cashAlternativeService.getWhere(req.params.whereClause)
        .then(cashAlternatives => res.json(cashAlternatives))
        .catch(next);
    }
    else {
...
function getById(req, res, next) {
    // users can get their own cashAlternative and admins can get any cashAlternative
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    cashAlternativeService.getById(req.params.id)
        .then(cashAlternative => cashAlternative ? res.json(cashAlternative) : res.sendStatus(404))
        .catch(next);
}
...
function create(req, res, next) {
    cashAlternativeService.create(req.body)
...
function update(req, res, next) {
    // users can update their own cashAlternative and admins can update any cashAlternative
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }

    cashAlternativeService.update(req.params.id, req.body)
        .then(cashAlternative => res.json(cashAlternative))
        .catch(next);
}

function _delete(req, res, next) {
    // users can delete their own cashAlternative and admins can delete any cashAlternative
    if (Number(req.params.id) !== req.user.id && req.user.role !== Role.Admin) {
        return res.status(401).json({ message: 'Unauthorized' });
    }
